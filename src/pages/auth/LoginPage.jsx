...
const LoginPage = () => {
  ...
  const handleVerifyOTP = async (otpCode = null) => {
    ...
    if (result.success) {
      ...
      // Check if there's a redirect URL from previous attempt
      const from = location.state?.from || '/dashboard';
      
      // Get user data from localStorage to check role
      const userData = JSON.parse(localStorage.getItem('user') || '{}');
      
      // Redirect based on user role or saved location
      if (from !== '/dashboard') {
        // If there was a specific page they were trying to access, go there
        navigate(from);
      } else if (userData.role === 'provider' && userData.provider_status === 'approved') {
